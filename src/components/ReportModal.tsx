    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-6" onClick={() => setReportOpen(false)}>
      <div className="brackets bg-void-900 border border-void-700 rounded-sm w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-void-100">Monthly Report</h3>
          <button onClick={() => setReportOpen(false)} className="text-void-500 hover:text-void-300 text-xl">&times;</button>
        </div>

        {!latest && <div className="text-void-500">No reports yet -- advance the clock to generate one.</div>}

        {latest && (
          <div className="brackets mb-6 border border-sage-800 rounded-sm p-4 bg-void-950/50">
            <div className="text-sage-400 font-semibold mb-2">{latest.headline}</div>
            ...
            {latest.bullets.length === 0 && <div className="text-void-500 text-sm">No major developments this month.</div>}
          </div>
        )}

        {reports.length > 1 && (
          <>
            <h4 className="text-sm uppercase tracking-wide text-void-500 mb-2">Recent History</h4>
            <div className="space-y-2">
              {reports.slice(1).map((r) => (
                <div key={`${r.date.year}-${r.date.month}`} className="text-sm border-l-2 border-void-800 pl-3">
                  <div className="text-void-400 font-medium">{formatDate(r.date)}</div>
                  {r.bullets.length > 0
                    ? <div className="text-void-500">{r.bullets[0]}{r.bullets.length > 1 ? ` (+${r.bullets.length - 1} more)` : ''}</div>
                    : <div className="text-void-600 italic">Quiet month.</div>}
                </div>
              ))}
            </div>
          </>
