  ...
  return (
    <div className="p-5 max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-lg p-4">
        <h3 className="text-slate-100 font-semibold mb-3">Government</h3>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-lg p-4">
        <h3 className="text-slate-100 font-semibold mb-3">Parties</h3>
        <div className="space-y-2">
          {p.parties.map((party) => (
            <div key={party.id} className="flex items-center gap-3 text-sm">
              <span className={`w-2 h-2 rounded-full ${party.inGovernment ? 'bg-amber-400' : 'bg-slate-600'}`} />
              <span className="flex-1 text-slate-200">{party.name}</span>
              <span className="text-slate-500 text-xs">{party.ideology}</span>
              <span className="text-slate-400 w-16 text-right tabular-nums">{party.seatSharePct.toFixed(0)}% seats</span>
              <span className="text-slate-400 w-16 text-right tabular-nums">{party.popularSupportPct.toFixed(0)}% support</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-lg p-4">
        <h3 className="text-slate-100 font-semibold mb-3">Policy Levers</h3>
